...
double Pk2 = 10;  //speed it gets there
double Ik2 = 0;
double Dk2 = 0;

double Setpoint2, Input2, Output2, Output2a;    // PID variables - front/back stability
...
double SetpointSide1, InputSide1, OutputSide1;    
...
double SetpointSide2, InputSide2, OutputSide2;    
...
double SetpointSide3, InputSide3, OutputSide3;    
...
double SetpointSide4, InputSide4, OutputSide4;   
...
int buttons;  //zipped up binary buttons
int but1 = 1;
int but2 = 1;
int but3 = 1;
int but4 = 1;
...
int legRot;  // leg rotation pot position
int openFlag = 0;
...
unsigned long openTime; // leg opening safety check
   ...
   unsigned long currentMillis = millis();
     if (currentMillis - previousMillis >= interval) {  //start timed event
        ...
        if (Serial1.available()>=24) {  // check for enough serial data
                ...
                */                
                ...
                if (ident == iam && ident2 == iam) {  // check data is for this device and in the right order
                    ...
                    buttons = read1;   // if the data is right update to main loop variables
                    but9 = read9;
                    slide1 = read10; 
                    pot1 = read11; 
                    pot2 = read12; 
                    ...
                    but1 = bitRead(buttons, 0);
                    but2 = bitRead(buttons, 1);
                    but3 = bitRead(buttons, 2);
                    but4 = bitRead(buttons, 3);
                    but5 = bitRead(buttons, 4);
                    but6 = bitRead(buttons, 5);
                    but7 = bitRead(buttons, 6);
                    but8 = bitRead(buttons, 7);
      ...
      if (millis() - lastDataTime <= 200 && millis() - lastDataTime2 <=200) {  //saftey check - proceed if data is current enough
              ...
              if (openFlag == 1 && legRot >= 575) {
                analogWrite(12,255);
                analogWrite(13,0);
              }

              else if (openFlag == 0 && legRot <= 580) {
                analogWrite(13,255);
                analogWrite(12,0);
              }
               ...
               if (Output1 < 0)                                      // decide which way to turn the wheels based on deadSpot variable
                 ...
                 c2 = OutputSide4*1;            // right hip  
                   ...
                   // ************ step timer ************
                    ...
                    // leg turning / splitting goes in here

                    // end of leg turning code
                     ...
                     if (but7 == 1) {
                      digitalWrite(25, LOW);  // play sound 1
                     }
                     else {
                     ...
                     if (but8 == 1) {
                      digitalWrite(27, LOW);  // play sound2
                     }
                     else {
