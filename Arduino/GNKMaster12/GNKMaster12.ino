   ...
   unsigned long currentMillis = millis();
     if (currentMillis - previousMillis >= interval) {  //start timed event
        ...
        if (Serial1.available()>=24) {  // check for enough serial data
                ...
                read4 = Serial1.parseInt();
                read5 = Serial1.parseInt();
                read6 = Serial1.parseInt();
                read7 = Serial1.parseInt();
                read8 = Serial1.parseInt();
      ...
      if (millis() - lastDataTime <= 200 && millis() - lastDataTime2 <=200) {  //saftey check - proceed if data is current enough
              ...
              Input1 = waist;
              Setpoint1 = map(Output2,255,-255,10,50);              
              PID1.Compute();         
              ...
              // Drive front/back stability motor
               ...
               if (Output1 < 0)                                      // decide which way to turn the wheels based on deadSpot variable
                 ...
                 SetpointSide1 = 0-abs(imuY/5);           // left ankle
                 SetpointSide2 = 0-abs(imuY/5);           // left hip
                 SetpointSide3 = 0+abs(imuY/5);           // right ankle
                 ...
                 c2 = OutputSide4*1;            // right hip  
                   ...
                   // ************ step timer ************
                     ...
                     if (but1 == 0 && stepFlag == 0) {
                      ...
                      if (stepFlag == 2 && currentMillis - previousStepTime >= stepTime) {
                       rightFlag = 1;
                       stepFlag = 3;
                       previousStepTime = currentMillis; // reset the clock
                      } 
                      
                      if (stepFlag == 3 && currentMillis - previousStepTime >= stepTime) {
                       stepFlag = 0;
                       previousStepTime = currentMillis; // reset the clock
                      }                    
                    ...
                    if (leftFlag == 2 && currentMillis - previousLeftMillis >= leftTime) {
                      b1 = 0;
                      b2 = 0;
                      leftFlag = 0;
                      previousLeftMillis = currentMillis; // reset the clock
                    ...
                    if (rightFlag == 2 && currentMillis - previousRightMillis >= rightTime) {
                      d1 = 0;
                      d2 = 0;
                      rightFlag = 0;
                      previousRightMillis = currentMillis; // reset the clock
